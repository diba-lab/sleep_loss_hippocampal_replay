...
def get_spike_positions(
    ...
    if channel_locations is not None:
        com = np.array(
            [
                np.sum((channel_locations[spike_feat_ind][:, :, 0] * pc_power), 1)
                / np.sum(pc_power, 1),
                np.sum((channel_locations[spike_feat_ind][:, :, 1] * pc_power), 1)
                / np.sum(pc_power, 1),
            ]
        ).T
    else:
...
def calculate_metrics(
    ...
    total_units = int(np.max(spike_clusters))
    total_epochs = len(epochs)

    for epoch in epochs:
        in_epoch = np.logical_and(
            spike_times >= epoch.start_time, spike_times < epoch.end_time
        ).flatten()
        ...
        # isi_viol = calculate_isi_violations(
        #     spike_times=spike_times[in_epoch],
        ...
        # presence_ratio = calculate_presence_ratio(
        #     spike_times=spike_times[in_epoch],
        #     spike_clusters=spike_clusters[in_epoch],
        #     total_units=total_units,
        #     duration=duration,
        ...
        # firing_rate = calculate_firing_rates(
        #     spike_times=spike_times[in_epoch],
        #     spike_clusters=spike_clusters[in_epoch],
        #     total_units=total_units,
        #     duration=duration,
        ...
        # amplitude_cutoff = calculate_amplitude_cutoff(
        #     spike_clusters=spike_clusters[in_epoch],
        ...
        #     min_spikes_per_interval=params["drift_metrics_min_spikes_per_interval"],
        #     channel_locations=channel_locations,
        #     verbose=verbose,
        # )
...
def calculate_isi_violations(
    ...
    for idx, cluster_id in enumerate(cluster_ids):
        ...
        viol_rates[cluster_id], num_violations = isi_violations(
...
def calculate_presence_ratio(
    ...
    for idx, cluster_id in enumerate(cluster_ids):
        ...
        ratios[cluster_id] = presence_ratio(
            spike_times[for_this_cluster], duration=duration
        )

    return ratios


def calculate_num_spikes(
    spike_times, spike_clusters, total_units, spike_cluster_subset=None, verbose=True
):
...
def calculate_firing_rates(
    ...
    for idx, cluster_id in enumerate(cluster_ids):
        ...
        for_this_cluster = spike_clusters == cluster_id
        firing_rates[cluster_id] = firing_rate(
            spike_times[for_this_cluster], duration=duration
        )
...
def calculate_amplitude_cutoff(
    spike_clusters, amplitudes, total_units, spike_cluster_subset=None, verbose=True
):
    if spike_cluster_subset is not None:
        cluster_ids = spike_cluster_subset
    else:
...
def calculate_pc_metrics(
    ...
    for idx, cluster_id in enumerate(all_cluster_ids):
        ...
        neighboring_channels[idx] = find_neighboring_channels(
            pc_feature_ind[idx, pc_max],
            pc_feature_ind[idx, :],
            num_channels_to_compare,
            channel_locations,
        )
    ...
    for idx, cluster_id in enumerate(cluster_ids):
        ...
        units_for_channel, channel_index = np.unravel_index(
            np.where(pc_feature_ind.flatten() == peak_channel)[0], pc_feature_ind.shape
        )

        # units_in_range: list of bool, True for units whose peak channels are in the neighborhood of target unit
        units_in_range = [
            channel in neighboring_channels[idx]
            for channel in peak_channels[units_for_channel]
        ]
        ...
        if spike_counts[this_unit_idx] > max_spikes_for_cluster:
            relative_counts = (
                spike_counts / spike_counts[this_unit_idx] * max_spikes_for_cluster
            )
        else:
        ...
        for idx2, cluster_id2 in enumerate(units_for_channel):
            ...
            try:
                channel_mask = make_channel_mask(
                    cluster_id2, pc_feature_ind, channels_to_use
                )
            except IndexError:
                ...
                index_mask = make_index_mask(
                    spike_clusters,
                    all_cluster_ids[cluster_id2],
                    min_num=0,
                    max_num=subsample,
                    seed=seed,
                )
        ...
        all_pcs = np.reshape(
            all_pcs, (all_pcs.shape[0], pc_features.shape[1] * channels_to_use.size)
        )
        ...
        if all_pcs.shape[0] > min_num_pcs:
            ...
            if "nearest_neighbor" in metric_names:
                nn_hit_rates[idx], nn_miss_rates[idx] = nearest_neighbors_metrics(
                    all_pcs, all_labels, cluster_id, spikes_for_nn, n_neighbors
                )
            else:
            ...
            print(
                f"Unit {str(cluster_id)} only has "
                + str(all_pcs.shape[0])
                + " spikes, which is not enough to compute metric; assigning nan..."
            )
            isolation_distances[idx] = np.nan
            l_ratios[idx] = np.nan
            d_primes[idx] = np.nan
...
def calculate_silhouette_score(
    ...
    random_spike_inds = np.random.RandomState(seed=seed).permutation(
        spike_clusters.size
    )
    ...
    for idx1, i in enumerate(cluster_ids):
        ...
        for idx2, j in enumerate(all_cluster_ids):
            if (
                (i, j) not in seen_unit_pairs
                and (j, i) not in seen_unit_pairs
                and i != j
            ):
...
def calculate_drift_metrics(
    ...
    interval_starts = np.arange(
        np.min(spike_times), np.max(spike_times), interval_length
    )
    ...
    for idx, cluster_id in enumerate(cluster_ids):
        ...
        if np.any(position_diffs > 0):
            max_drift[cluster_id] = np.around(
                np.max(position_diffs[position_diffs > 0]), 2
            )
            # The +1 diagonal contains the step-by-step drifts between intervals.
            # Summing them up we obtain cumulative drift
            cumulative_drift[cluster_id] = np.around(
                np.sum(np.diag(position_diffs, 1)), 2
            )
        else:
            # not enough spikes
...
def nearest_neighbors_metrics(
    all_pcs, all_labels, this_unit_id, spikes_for_nn, n_neighbors
):
...
def find_neighboring_channels(
    peak_channel, channel_list, num_channels_to_compare, channel_locations
):
    """
    ...
    distances = [
        np.linalg.norm(peak_channel_location - loc) for loc in channel_locations
    ]
